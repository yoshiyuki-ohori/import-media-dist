...
完了しました"
  ...
  if [[ $copied -gt 0 ]]; then
    ...
    # New files = 0 (card re-inserted, everything already imported). Show only a
    # local notification on this Mac — do NOT ping the team LINE, to avoid noise
    # every time an already-imported card is plugged in/out.
